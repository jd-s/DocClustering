...
def getClusterName (G, color, maxCount=15, separator=""):
	meshTags = []
	colorList=nx.get_node_attributes(G,'color')
	...
	for key, value in sorted(countTerms.items(), key=operator.itemgetter(1), reverse=True):
		returnString += str(key) + " ("+str(int(oldList[key]))+") "+separator
		#returnString += str(key) + " ("+str(oldList[key] )+")"
		i += 1
	return (returnString)

# def getClusterName (G, color, logging, maxCount=3, separator="", normalize=False):
# 	meshTags = []
# 	#print (meshList)
# 	colorList=nx.get_node_attributes(G,'color')
...
# 				meshTags.extend (meshList.get(node))
# 		if isinstance(meshList.get(node), str):
# 			allmeshTags.append (meshList.get(node))
# 		else:
# 			allmeshTags.extend (meshList.get(node))
# 	#print (meshTags)
...
# 	countTerms = Counter(meshTags)
# 	countAllTerm = Counter(allmeshTags)
# 	oldList = countTerms
# 	#print (str(oldList))
...
# 		if normalize:
# #			logging.info (" Doing normalization "+str(value) +"/"+ str(countAllTerm[key]))
# 			countTerms[key] = float(value) / countAllTerm[key]
# 		else:
# 			countTerms[key] = value
# 	#print ( sorted(countTerms.items(), key=operator.itemgetter(1), reverse=True) )
#
...
# 	for key, value in sorted(countTerms.items(), key=operator.itemgetter(1), reverse=True):
# 		#print (key)
# 		returnString += key + " ("+str(countTerms[key])+") "+separator
# 		i += 1
# 		if (i > 15) and (maxCount != -1):
# 			break;
# 	#print (returnString)
# 	return (returnString)
...
def buildValueGraph (G, d, count=15, normalize=True):
	valueGraph=nx.Graph()
	...
	for i in range(0,colorCount):
		...
		# Blabla
		journals = getClusterJournals (G,i,count2)
